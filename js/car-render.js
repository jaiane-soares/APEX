...
window.changeColor = (colorHex) => {
    if (!carViewer.model) {
        console.warn("Aguardando modelo carregar...");
        return;
    }
    ...
    carViewer.model.materials.forEach((material) => {
        // Incluído suporte para material 'rodas_GSi'
        if (material.name.toLowerCase().includes('carro_') || material.name.includes('rodas_GSi')) {
